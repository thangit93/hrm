...
class PayrollDataAdapter extends TableEditAdapter {
    ...
    preProcessTableData(row) {
        $.each(row, function (index, cell) {
            if (!isNaN(cell)) {
                cell = Intl.NumberFormat('de-DE').format(cell);
                row[index] = `${cell}`;
            }
        })
        return row;
    ...
    addAdditionalRequestData(type, req) {
        if (type === 'updateData') {
            ...
            req.payrollId = this.payrollId;
        } else if (type === 'exportAllData1') {
            req.payrollId = this.payrollId;
            req.export = "1";
        } else if (type === 'exportAllData2') {
            req.payrollId = this.payrollId;
            req.export = "2";
    ...
    downloadPayroll() {
        ...
        element.setAttribute('href', `data:text/plain;charset=utf-8,%EF%BB%BF${encodeURIComponent(this.getCSVData())}`);
        ...
        const rangeAmounts = ['rangeAmounts', {
            ...
            form: [
                ['lowerCondition', {
                    ...
                    type: 'select',
                    source: [['No Lower Limit', 'No Lower Limit'], ['gt', 'Greater than'], ['gte', 'Greater than or Equal']]
                }],
                ['lowerLimit', {label: 'Lower Limit', type: 'text', validation: 'float'}],
                ['upperCondition', {
                    label: 'Upper Limit Condition',
                    type: 'select',
                    source: [['No Upper Limit', 'No Upper Limit'], ['lt', 'Less than'], ['lte', 'Less than or Equal']]
                }],
        ...
        const payslipFields = ['data', {
            ...
            form: [
                ['type', {
                    ...
                    type: 'select',
                    sort: 'none',
                    source: [['Payroll Column', 'Payroll Column'], ['Text', 'Text'], ['Company Name', 'Company Name'], ['Company Logo', 'Company Logo'], ['Separators', 'Separators']],
                }],
                ['payrollColumn', {
